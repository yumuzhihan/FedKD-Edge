...
class CheckpointState:
    ...
    @classmethod
    def from_server(cls, server: "FederatedServer", round_idx: int) -> "CheckpointState":
...
class CheckpointManager:
    ...
    CHECKPOINT_PATTERN = re.compile(
        r"checkpoint_(?P<strategy>\w+)_(?P<dataset>\w+)_seed(?P<seed>\d+)_round(?P<round>\d+)_\d+\.pth"
    )
    ...
    def validate_checkpoint(
        ...
        # 关键字段必须完全匹配
        critical_fields = ["strategy", "dataset", "seed", "num_users", "frac"]
